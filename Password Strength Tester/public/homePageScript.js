...
const passwordStrengthTesterDOM = document.getElementsByClassName('password-strength-tester');
const strengthTestButton = document.getElementById('btn-strength-test');
const recordsDOM = document.getElementById('password-records-container');
const recordTableBodyDOM = document.getElementById('record-table-body');
...
const showRecords = async () => {
    const {data: 
        {records}
    } = await axios.get('http://localhost:5000/api/v1/passwordManager/showAllPassword');
    ...
    for (let key in record) {
        ...
        if(record.hasOwnProperty(key)) {
            ...
            row.addEventListener('click', () => showRecord(nameCell.textContent));
        }
      }
    }
