...
const storePassword = async (req, res) => {
    ...
    const doesDomainAlreadyExists = await PasswordManager.findOne({user: req.user.userId, name: name});
    if(doesDomainAlreadyExists) {
        throw new CustomError.BadRequestError('Password for domain already set');
    }
    const record = await PasswordManager.create({ name, password, user });
...
const showPassword = async (req, res) => {
    ...
    if (!domainName) {
       throw new CustomError.BadRequestError('Please provide domain name');
    }
    const record = await PasswordManager.findOne({user: req.body.user, name: domainName});
...
const showAllPasswords = async (req, res) => {
    ...
    if(!records) {
        throw new CustomError.NotFoundError('No records available for the user')
    }
...
const updatePassword = async (req, res) => {
    ...
    if(!passwordId) {
        throw new CustomError.NotFoundError(`Record of password with id: ${passwordId} doesnot exist`)
    }

    const record = await PasswordManager.findOne({user: userId, _id: passwordId});
...
const deletePassword = async (req, res) => {
    ...
    const record = await PasswordManager.findOneAndDelete({_id: passwordId});
if(!record) {
    throw new CustomError.NotFoundError(`No record of password with id: ${passwordId}`)
}
